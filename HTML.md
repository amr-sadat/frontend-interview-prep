...
- [🟢 Q8 - What are semantic HTML elements and why are they important?](#q8-what-are-semantic-html-elements-and-why-are-they-important)
- [🟢 Q9 - What is the purpose of the `meta viewport` tag?](#q9-what-is-the-purpose-of-the-meta-viewport-tag)
- [🟡 Q10 - What is the difference between `async` and `defer` on script tags?](#q10-what-is-the-difference-between-async-and-defer-on-script-tags)
- [🟢 Q11 - What is the difference between `<input type="button">` and `<button>`?](#q11-what-is-the-difference-between-input-typebutton-and-button)
- [🟡 Q12 - How does the browser parse and render HTML? Explain the critical rendering path.](#q12-how-does-the-browser-parse-and-render-html-explain-the-critical-rendering-path)
- [🟡 Q13 - What are Web Components?](#q13-what-are-web-components)
- [🟡 Q14 - What is the difference between `localStorage`, `sessionStorage`, and cookies?](#q14-what-is-the-difference-between-localstorage-sessionstorage-and-cookies)
- [🟡 Q15 - How do you optimize image loading in HTML?](#q15-how-do-you-optimize-image-loading-in-html)
- [🟡 Q16 - What is progressive enhancement vs graceful degradation?](#q16-what-is-progressive-enhancement-vs-graceful-degradation)
- [🟡 Q17 - What are `<header>`, `<nav>`, `<main>`, `<aside>`, and `<footer>` used for?](#q17-what-are-header-nav-main-aside-and-footer-used-for)
- [🟡 Q18 - Why should you use `<button>` instead of a clickable `<div>`?](#q18-why-should-you-use-button-instead-of-a-clickable-div)
- [🟡 Q19 - What is the difference between GET and POST?](#q19-what-is-the-difference-between-get-and-post)
- [🟡 Q20 - What are `action` and `method` in a form?](#q20-what-are-action-and-method-in-a-form)
- [🟡 Q21 - What is the purpose of the `<label>` element?](#q21-what-is-the-purpose-of-the-label-element)
- [🟡 Q22 - What is the difference between `disabled`, `readonly`, and `required`?](#q22-what-is-the-difference-between-disabled-readonly-and-required)
- [🟡 Q23 - What is the `name` attribute used for in form inputs?](#q23-what-is-the-name-attribute-used-for-in-form-inputs)
- [🟡 Q24 - How does HTML form validation work?](#q24-how-does-html-form-validation-work)
- [🟡 Q25 - What are the major features introduced in HTML5?](#q25-what-are-the-major-features-introduced-in-html5)
- [🟡 Q26 - What is the difference between Canvas and SVG?](#q26-what-is-the-difference-between-canvas-and-svg)
- [🟡 Q27 - What are `<audio>` and `<video>` used for?](#q27-what-are-audio-and-video-used-for)
- [🟡 Q28 - What are `<picture>` and `srcset` used for?](#q28-what-are-picture-and-srcset-used-for)
- [🟡 Q29 - What is lazy loading and how does `loading="lazy"` work?](#q29-what-is-lazy-loading-and-how-does-loadinglazy-work)
- [🔴 Q30 - How can you optimize HTML page performance?](#q30-how-can-you-optimize-html-page-performance)
- [🔴 Q31 - What is web accessibility?](#q31-what-is-web-accessibility)
...
Neither carries semantic meaning — prefer semantic tags (see Q8) when a more meaningful element fits.
...
### Q8. What are semantic HTML elements and why are they important?
...
### Q9. What is the purpose of the `meta viewport` tag?
...
### Q10. What is the difference between `async` and `defer` on script tags?
...
### Q11. What is the difference between `<input type="button">` and `<button>`?
...
### Q12. How does the browser parse and render HTML? Explain the critical rendering path.
...
5. **JS execution — bidirectional blocking:**
   - JS is **parser-blocking** by default: a `<script>` tag pauses HTML parsing until the script downloads and runs — because scripts can use `document.write()` or otherwise mutate the DOM being built (`async` downloads in parallel and runs whenever ready; `defer` downloads in parallel but runs after parsing, in order — see Q10).
...
### Q14. What is the difference between `localStorage`, `sessionStorage`, and cookies?
...
### Q15. How do you optimize image loading in HTML?
...
### Q16. What is progressive enhancement vs graceful degradation?
...
### Q17. What are `<header>`, `<nav>`, `<main>`, `<aside>`, and `<footer>` used for?
...
### Q18. Why should you use `<button>` instead of a clickable `<div>`?
...
### Q20. What are `action` and `method` in a form?
...
- **`method`** — the HTTP method used to send the data — typically GET or POST (see Q19).
...
On submit, the browser sends a request to `/submit-login` using POST, with the form's input values in the request body (keyed by each input's `name` attribute — see Q23).

---

### Q21. What is the purpose of the `<label>` element?
...
### Q22. What is the difference between `disabled`, `readonly`, and `required`?
...
### Q23. What is the `name` attribute used for in form inputs?
...
### Q24. How does HTML form validation work?
...
### Q25. What are the major features introduced in HTML5?
...
### Q26. What is the difference between Canvas and SVG?
...
### Q27. What are `<audio>` and `<video>` used for?
...
### Q28. What are `<picture>` and `srcset` used for?
...
### Q29. What is lazy loading and how does `loading="lazy"` work?
...
### Q30. How can you optimize HTML page performance?
...
- Compress images (WebP/AVIF), use `srcset`/`<picture>` to serve appropriately sized images (see Q28)
- Lazy-load offscreen images/iframes with `loading="lazy"` (but not above-the-fold content — see Q29)

**Control script loading**
- Use `defer` or `async` on `<script>` tags so parsing isn't blocked (see Q10)
...
Practically, accessibility in HTML comes from: semantic elements (see Q8), proper alt text (see Q32), labeled forms (see Q21, Q35), sufficient color contrast, keyboard operability (see Q36), and ARIA where semantics fall short (see Q33).
...
```
Never rely on `placeholder` alone — it disappears on focus and isn't reliably read by all assistive tech (see Q21).
